...
`recording.py` — **portillon de conservation**, pur. Le direct s'affiche toujours ; l'écriture disque attend l'accord, et l'attente (bornée) est versée à ce moment-là, **chaque phrase avec l'heure où elle a été dite** (sinon tout prenait l'heure du clic : ligne de temps effondrée, SRT de durée nulle) — et la réunion qu'elle ouvre commence à cette heure-là. Verrouillé : `add` vient du thread STT, `arm`/`reset` du thread Qt. **Vaut aussi en mode remote** (`app.consent`, passé au client distant à la place de l'historique). Ne logue que des comptes, jamais du texte.
...
`speaker_intro.py` — **pur** : reconnaît qui se présente (« je m'appelle Guillaume », « moi c'est Léa », « my name is… ») pour nommer son étiquette de locuteur. Première personne seulement, majuscule exigée, « je suis » exclu : un faux nom collé toute une réunion est pire qu'un « A ». Câblé par `app._on_display_event` sur le bus d'affichage : même chemin qu'un renommage manuel (Live → overlay), **jamais** d'écrasement d'un nom déjà posé, persistance dans `meetings.json` seulement une fois l'accord de conservation donné. Ni le nom ni le texte ne sont loggués.

`meetings.py` — registre des réunions (id, titre, début, fin, **noms de locuteurs**, **moments marqués**) dans `meetings.json` (0600, écriture atomique). La réunion courante est un état **du process** (plusieurs `TranscriptionHistory` écrivent le même fichier) et n'est ouverte que par la première transcription : `current_meeting_id()` lit sans créer. Effacer une réunion passe par `delete_meeting()`, qui oublie aussi la réunion courante si c'est elle (sinon la suite s'écrivait sous un identifiant effacé) ; ses résumés partent avec elle (`summarizer.delete_summaries`, index `summaries/.meetings.json`).
...
`account.py` — compte Benji côté app : `Session` (login/register/refresh auto) + persistance des jetons dans `credentials.json` (0600) sous les données utilisateur. L'abonnement suit le **compte**, pas le poste → mêmes identifiants = même plan sur toute plateforme. Au démarrage, `main.py` injecte l'access token dans `LLMConfig.backend_token` — mais il expire en 15 min : tout appel au backend passe par `Session.access_token` (STT distant, résumé remote, facturation), jamais par ce jeton figé. Le rafraîchissement est **sérialisé** (refresh rotatif + détection de réutilisation côté backend : deux refresh simultanés révoquaient toute la session) et **ne déconnecte que sur un refus** (4xx) — une coupure réseau ou un 5xx rend `None` et garde la session.
